$(document).ready(function() {
    ...
    var svg = d3.select("#barGraph2").append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");
    $(function() {
        $.get('/auth/grafico-noticias-equipo', function(data) {
            ...
            svg.selectAll("bar").data(data).enter().append("rect").attr("class", "bar").attr("x", function(d) {
                ...
                return y(d.cantnoticias);
            }).attr("height", function(d) {
                return height - y(d.cantnoticias);
            });
        });
    });
});